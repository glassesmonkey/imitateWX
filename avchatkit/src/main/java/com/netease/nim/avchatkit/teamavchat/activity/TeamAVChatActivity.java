...
public class TeamAVChatActivity extends UI {
    ...
    /**
     * **************************************** //imitate weixin 处理群扩展字段 ********************************************
     */
    private void modifyTeamExt(String teamId,Boolean hangup,String roomId) {
        //sortExt是对群扩展字段的处理
        sortExt(hangup,roomId);
        //提交修改的扩展字段
        NIMClient.getService(TeamService.class).updateTeam(teamId, TeamFieldEnum.Extension, ext).setCallback(new RequestCallback<Void>() {
            @Override
    ...
    private void sortExt(boolean hangup,String roomId) {
        String account = AVChatKit.getAccount();
        LogUtil.d("gaotest: ",ext);
        //判断是否是挂断，如果挂断则去除自己的账号，如果是接听则加入自己的账号进扩展字段
        if(hangup){
            List<String> arrList = Arrays.asList(ext.split(","));
            List joinList = new ArrayList(arrList);
            for (int i = 0; i < joinList.size(); i++) {
                String s = (String) joinList.get(i);
                if (s.equals(account)) {
                    joinList.remove(s);
                }
            }
